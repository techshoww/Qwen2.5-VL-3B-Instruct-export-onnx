...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        head_dim = config.hidden_size // config.num_heads                   # 1280//16=80
    ...
    def rot_pos_emb(self, grid_thw):
        ...
        print("pos_ids",pos_ids)  # 顺序 和 Qwen2_5_VLImageProcessor 中 image patch的编排方式有关
        # tensor([[ 0,  0],
        # [ 0,  1],
        # [ 1,  0],
        # [ 1,  1],
        ...
        print("inv_freq",self.rotary_pos_emb.inv_freq)      # 对应 RoPE中的 m*theta
        print("max_grid_size",max_grid_size)
        rotary_pos_emb_full = self.rotary_pos_emb(max_grid_size)
        print("rotary_pos_emb_full",rotary_pos_emb_full.shape)      # [146, 20]
        # 图片是二维的所以每个patch的编码用两个编码表示 
        rotary_pos_emb = rotary_pos_emb_full[pos_ids].flatten(1)    # shape: [14308, 40]
    ...
    def get_window_index(self, grid_thw):
        ...
        window_index_id = 0
        vit_merger_window_size = self.window_size // self.spatial_merge_size // self.patch_size  # spatial_merge_size 相当于进行下采样
        print("vit_merger_window_size",vit_merger_window_size)  # 每个window中的patch 个数
        # grid_thw  [[  1,  98, 146]]
        for grid_t, grid_h, grid_w in grid_thw:
            # spatial_merge_size=2, patch 2x2 采样为1个
            ...
            print("index",index.shape)  # [1, 49, 73]
            pad_h = vit_merger_window_size - llm_grid_h % vit_merger_window_size
            pad_w = vit_merger_window_size - llm_grid_w % vit_merger_window_size
            num_windows_h = (llm_grid_h + pad_h) // vit_merger_window_size
            num_windows_w = (llm_grid_w + pad_w) // vit_merger_window_size
            print("num_windows_h",num_windows_h)        # 13
            print("num_windows_w",num_windows_w)        # 19
            index_padded = F.pad(index, (0, pad_w, 0, pad_h), "constant", -100) 
            index_padded = index_padded.reshape(
                grid_t,
                num_windows_h,                          # 水平方向 window个数
                vit_merger_window_size,                 # 每个 window中的patch个数
                num_windows_w,
                vit_merger_window_size,
            )
            print("index_padded.shape",index_padded.shape)              #[1, 13, 4, 19, 4]  
            index_padded = index_padded.permute(0, 1, 3, 2, 4).reshape(
                grid_t,
                num_windows_h * num_windows_w,
                vit_merger_window_size,
                vit_merger_window_size,
            )                                                       # num_windows_h * num_windows_w 表示window的个数，vit_merger_window_size表示每个window中的patch 个数
            print("index_padded.shape",index_padded.shape)      # [1, 247, 4, 4]
            print("index_padded",index_padded)                  # 按window 编排patch index
        #     tensor([[[[   0,    1,    2,    3],
        #   [  73,   74,   75,   76],
        #   [ 146,  147,  148,  149],
        #   [ 219,  220,  221,  222]],
        ...
        #   [-100, -100, -100, -100],
        #   [-100, -100, -100, -100]]]]
            seqlens = (index_padded != -100).sum([2, 3]).reshape(-1)    # 每个window中的patch个数
            index_padded = index_padded.reshape(-1)
            index_new = index_padded[index_padded != -100]
            window_index.append(index_new + window_index_id)
            print("seqlens",seqlens)
            # 前面patch 进行了 2x2 采样 1 个，这里 乘上spatial_merge_unit 进行恢复。每个window中4*4*2*2=64个patch，即 112/14*112/14=64
            # + cu_window_seqlens[-1]， 如果有下一帧图片，patch index 跟着 上一帧最大index 递增
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        torch.save(grid_thw, "grid_thw.pth")
...
__all__ = ["Qwen2_5_VLForConditionalGeneration", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel", "Qwen2_5_VLForConditionalGenerationExport"]
